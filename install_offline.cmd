...
"%TrustedPowerShell%" -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command ^
  ...
  "$dangerous=@('.exe','.com','.bat','.cmd','.ps1','.psm1','.vbs','.js','.jse','.wsf','.wsh','.scr','.msi');$allowedRootScripts=@('install_offline.cmd','uninstall_offline.cmd');foreach($entry in [IO.Directory]::EnumerateFileSystemEntries($root)){if(([IO.File]::GetAttributes($entry)-band [IO.FileAttributes]::ReparsePoint)-ne 0){throw ('Unexpected reparse point at bundle root: '+$entry)};if([IO.File]::Exists($entry)-and $dangerous-contains [IO.Path]::GetExtension($entry).ToLowerInvariant()){if($allowedRootScripts-inotcontains [IO.Path]::GetFileName($entry)){throw ('Unexpected executable or script at bundle root: '+$entry)}}};" ^
  ...
  "$manifestKey=$manifestPath.ToLowerInvariant();if($seen.Count-ne ($manifestSeen.Count+1)){throw 'Checksum inventory and manifest file set differ'};foreach($key in $seen.Keys){if($key-ne $manifestKey-and-not $manifestSeen.ContainsKey($key)){throw 'Checksum entry is absent from manifest'}};$manifestDigest=$seen[$manifestKey];if([string]::IsNullOrWhiteSpace($directoryLockScript)){throw 'Authenticated bundle-directory lock helper is unavailable'};$bootstrapPath=[IO.Path]::GetFullPath((Join-Path $root 'install_offline.cmd'));$bootstrapKey=$bootstrapPath.ToLowerInvariant();$stream=[IO.File]::Open($bootstrapPath,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::Read);try{$actual=Get-Sha256 $stream;if($actual-ne $seen[$bootstrapKey]){throw ('Bootstrap path changed before directory locking: '+$bootstrapPath)};$lockedFiles.Add($stream);$stream=$null}finally{if($null-ne $stream){$stream.Dispose()}};. ([scriptblock]::Create($directoryLockScript));$lockedDirectories=@(Lock-BundleDirectoryPaths $root @($seenPaths.Values));$actualFiles=@([IO.Directory]::EnumerateFiles($root,'*',[IO.SearchOption]::AllDirectories)|ForEach-Object{[IO.Path]::GetFullPath($_).ToLowerInvariant()});$expectedFiles=@($seen.Keys)+$checksumPath.ToLowerInvariant();$unexpected=@($actualFiles|Where-Object{-not $seen.ContainsKey($_)-and $_-ne $checksumPath.ToLowerInvariant()});if($actualFiles.Count-ne $expectedFiles.Count-or $unexpected.Count-ne 0){throw ('Bundle source tree differs from the verified manifest: '+($unexpected-join ', '))};foreach($key in @($seen.Keys)){$lockedPath=$seenPaths[$key];$stream=[IO.File]::Open($lockedPath,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::Read);try{$actual=Get-Sha256 $stream;if($actual-ne $seen[$key]){throw ('Bundle payload path changed before elevation: '+$lockedPath)};$lockedFiles.Add($stream);$stream=$null}finally{if($null-ne $stream){$stream.Dispose()}}};Write-Host 'Initial SHA-256 verification passed.';Write-Host 'Protected payload files and paths are locked during installation.';" ^
  "$env:DICOMXPHITS_VERIFIED_STAGE=[Guid]::NewGuid().ToString('N');$env:DICOMXPHITS_BUNDLE_MANIFEST_SHA256=$manifestDigest;& (Join-Path $root 'tools\install_offline_verified.ps1');$exitCode=$LASTEXITCODE;if($null-eq $exitCode){$exitCode=0}" ^
  "}catch{Write-Error $_}finally{[Environment]::SetEnvironmentVariable('DICOMXPHITS_VERIFIED_STAGE',$null,'Process');[Environment]::SetEnvironmentVariable('DICOMXPHITS_BUNDLE_MANIFEST_SHA256',$null,'Process');foreach($stream in $lockedFiles){$stream.Dispose()};foreach($handle in $lockedDirectories){$handle.Dispose()}};exit $exitCode"
