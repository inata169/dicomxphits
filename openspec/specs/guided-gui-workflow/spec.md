...
state. It SHALL obtain progress only from the documented summary path inside
the selected workspace, bind displayed updates to the invocation started by
that GUI action, and keep the Tk event loop responsive.
...
time and finish time MUST be labelled approximate. The presentation MUST NOT
claim batch-level progress, statistical convergence, or a PHITS output value
