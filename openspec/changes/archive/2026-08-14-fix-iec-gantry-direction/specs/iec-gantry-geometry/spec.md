...
For the public head-first-supine fixed-field workflow at zero couch angle, a DICOM Gantry Angle `g` SHALL define the IEC-consistent patient-axis geometry. A positive source-axis distance `S` centimetres and
isocenter `I` SHALL produce PHITS source position
`q = (-S sin(g), 0, -S cos(g))` and unit beam direction
`d = (sin(g), 0, cos(g))`.
...
The PHITS source position, PHITS source direction, and accelerator `tr3` transform SHALL implement one mutually consistent geometry. The source SHALL
